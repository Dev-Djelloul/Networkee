...
function renderAvatar(string $username, string $size = '', string $imageUrl = '', bool $openToWork = false, string $baseUrl = '../'): string {
    ...
    // Le statut "open to work" est affiché via le .otw-banner juste sous la
    // photo (pas un badge miniature sur l'avatar).
    return $html;
}
...
function hasUserSaved(int $postId, int $userId, PDO $pdo): bool {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM saved_posts WHERE post_id = :post_id AND user_id = :user_id");
    $stmt->execute(['post_id' => $postId, 'user_id' => $userId]);
    return $stmt->fetchColumn() > 0;
}
...
function getApplicants(int $jobOfferId, PDO $pdo): array {
    // u.id est listé après ja.* pour écraser la colonne "id" (celle de la candidature) et
    // laisser l'id de l'utilisateur sous cette clé — c'est ce que renderHoverList() attend.
    $stmt = $pdo->prepare(
        "SELECT ja.*, u.id, u.username, u.profile_image, u.job_title, u.location
